...
var base_url = 'https://api.dovewallet.com/v1.1/public';
  ...
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
      ...
      if (body.message) {
        ...
        summary['High'] = parseFloat(body.result[0]['High']).toFixed(8);
        summary['Low'] = parseFloat(body.result[0]['Low']).toFixed(8);
        summary['Volume'] = parseFloat(body.result[0]['Volume']).toFixed(8);
        summary['Bid'] = parseFloat(body.result[0]['Bid']).toFixed(8);
        summary['Ask'] = parseFloat(body.result[0]['Ask']).toFixed(8);
        summary['PrevDay'] = parseFloat(body.result[0]['PrevDay']).toFixed(8);
        summary['Last'] = parseFloat(body.result[0]['Last']).toFixed(8);
